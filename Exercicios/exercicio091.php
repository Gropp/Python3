<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8"/>
	<link rel="stylesheet" type="text/css" href="_css/estilo.css">
	<title>PHP CURSO ONLINE - FORMULÁRIOS IF ANINHADOS</title>
</head>
<body>
<div>
    <?php
        // como o nome dada a variavel no metodo input era v, (name="v"), no php voce usa o metodo GET para "capturar" essa variavel e utiliza-la dentro do codigo de acordo com a sua necessidade!
        $a = isset($_GET["ano"])?$_GET["ano"]:1900;
        $i = date("Y") - $a;
        echo "<p>Você nasceu em $a e tem $i ano(s).</p>";
        if ($i < 16) {
            $tipoVoto = "não votam";
        }
        //if (($i >= 16 && $i < 18) || ($i > 65)) { - essa seria outra opcao de testar a idade acima de 65 anos
        //usando o elseif
        /*
            else {
                if($i >= 18 && $i < 65) {
                    $tipoVoto = "votam obrigatóriamente";     
                }
                else {
                    $tipoVoto = "não são obrigadas a votar"; 
                }
            } 
        */
        elseif ($i >= 16 && $i < 18) {
                $tipoVoto = "votam opcionalmente";  
            }
            elseif($i >= 18 && $i < 65) {
                    $tipoVoto = "votam obrigatóriamente";     
                }
                else {
                    $tipoVoto = "não são obrigadas a votar"; 
                } 
        echo "<p>Com essa idade, as pessoas $tipoVoto.</p>";
    ?>
    <a href="formulario.html">Voltar</a>
</div>
</body>
</html>